...
export default function Home() {
  ...
  return (
    <main className="flex-1 bg-background overflow-hidden relative">
      {/* Section 1: Hero */}
      <section ref={sectionRef} className="relative min-h-screen flex flex-col overflow-x-hidden will-change-transform pb-20">
        {/* Navbar */}
          ...
          <div className="flex items-center gap-12 md:gap-20">
            ...
            <Link href="/" className="flex items-center gap-3 group">
              <div className="relative w-8 h-8 rounded-lg overflow-hidden border border-white/5 transition-transform group-hover:scale-105">
                <Image src="/logo.png" alt="Neuralyn Logo" fill className="object-cover" />
              </div>
              <span className="text-xl font-bold tracking-tight text-foreground">Neuralyn</span>
            </Link>
            
            {/* Nav Links */}
            <div className="hidden md:flex items-center gap-1 text-sm font-medium text-muted-foreground">
              <Link href="#" className="px-3 py-2 hover:text-foreground transition-colors rounded-md hover:bg-white/5">Home</Link>
              <Link href="#" className="flex items-center px-3 py-2 hover:text-foreground transition-colors rounded-md hover:bg-white/5 group">
                Services <ChevronDown className="ml-1 w-4 h-4 opacity-70 group-hover:opacity-100 transition-opacity" />
              </Link>
              <Link href="#" className="px-3 py-2 hover:text-foreground transition-colors rounded-md hover:bg-white/5">Reviews</Link>
              <Link href="#" className="px-3 py-2 hover:text-foreground transition-colors rounded-md hover:bg-white/5">Contact us</Link>
            </div>
          </div>
          
          <Link href="#" className="bg-foreground text-background px-4 py-2 rounded-lg text-sm font-semibold hover:opacity-90 transition-opacity">
            Sign In
          </Link>
        </nav>
        ...
        >
          ...
          >
            ...
            <span className="text-sm font-medium text-muted-foreground mr-1">Say Hello to Corewave v3.2</span>
          </motion.div>
          ...
          >
            Your Insights.<br />
            One Clear <span className="font-serif italic font-normal pr-1">Overview.</span>
          </motion.h1>
          ...
          <motion.button 
            ...
            whileTap={{ scale: 0.98 }}
            transition={{ type: "spring", stiffness: 400, damping: 25, duration: 0.6, delay: 0.3 }}
      ...
      <section ref={testimonialRef} className="relative min-h-screen flex flex-col justify-center py-24 md:py-32 px-8 md:px-28 z-40 bg-background overflow-hidden">
        <div className="max-w-3xl mx-auto flex flex-col items-start gap-10 w-full relative z-10">
          ...
          <h2 className="text-4xl md:text-5xl font-medium leading-[1.2] flex flex-wrap will-change-auto">
            {testimonialWords.map((word, i) => {
              ...
              const opacity = useTransform(testimonialScrollYProgress, [start, end], [0.2, 1]);
              // eslint-disable-next-line react-hooks/rules-of-hooks
              const color = useTransform(testimonialScrollYProgress, [start, end], ["#595959", "#FFFFFF"]);
